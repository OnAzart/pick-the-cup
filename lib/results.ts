...
// Resolves real, already-decided tournament state from Postgres into the
// bracket's local shape. Shared by /api/locked (locks the bracket UI) and
// /api/leaderboard (scores saved predictions against reality).
...
export async function resolveLocked(): Promise<LockedData> {
  await ensureSchema();
